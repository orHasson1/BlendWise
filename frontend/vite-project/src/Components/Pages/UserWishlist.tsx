...
import { Droplets, Heart, Filter, Search, X } from 'lucide-react';
...
const UserWishlist: React.FC = () => {
  ...
  useEffect(()=>{
    ...
    client.get('/oil-relations/wishlist/')
      .then(res => { if(!mounted) return; const arr = res.data?.oils || []; setOils(arr); setWishlistIds(arr.map((o:Oil)=>o.id)); })
      .catch(e => { if(!mounted) return; setError(e?.message || 'Failed to load wishlist'); })
      .finally(()=> mounted && setLoading(false));
    return ()=>{ mounted = false; };
  }, []);

  // Summary load for owned list to reflect mutual exclusivity
  useEffect(()=>{
    let mounted = true;
    client.get('/oil-relations/summary/')
      .then(res => { if(!mounted) return; setOwnedIds(res.data?.owned || []); })
      .catch(()=>{})
    return ()=>{ mounted = false; };
  }, []);
  ...
  const handleStatusChange = (oil:Oil) => (u:{wishlist?:boolean; owned?:boolean}) => {
    if(u.wishlist !== undefined) {
      ...
      if(!u.wishlist) { // removed from wishlist: remove from page list
    ...
    if(u.owned !== undefined) {
      ...
      if(u.owned) { // moved to owned, drop from wishlist view
        setWishlistIds(prev => prev.filter(id=>id!==oil.id));
        setOils(prev => prev.filter(x=>x.id!==oil.id));
      }
    }
  };
  ...
  return (
    <div className="py-8">
      <PageHeader
        ...
        subtitle={oils.length ? `${oils.length} saved oil${oils.length===1?'':'s'}` : 'Save oils you want to try next.'}
      ...
      />
      {loading && (
        <div className="oil-grid mb-6" aria-label="Loading wishlist oils">
          ...
          <div className="flex-1 min-w-0">
            ...
            {(search || selectedNotes.length+selectedAromas.length+selectedVibes.length>0) && (
              <div className="flex flex-wrap gap-2 mb-6 items-center">
                ...
                {selectedNotes.map(id => (
                  <Badge key={`wish-chip-note-${id}`} variant="primary" className="flex items-center gap-1 pr-1">{optionLabel(noteOptions, id)}<button className="ml-0.5 inline-flex h-4 w-4 items-center justify-center rounded hover:bg-teal-600/15" onClick={()=>toggleFilter(setSelectedNotes, selectedNotes, id)}><X className="h-3 w-3" /></button></Badge>
                ))}
                {selectedAromas.map(id => (
                  <Badge key={`wish-chip-aroma-${id}`} variant="primary" className="flex items-center gap-1 pr-1">{optionLabel(aromaOptions, id)}<button className="ml-0.5 inline-flex h-4 w-4 items-center justify-center rounded hover:bg-teal-600/15" onClick={()=>toggleFilter(setSelectedAromas, selectedAromas, id)}><X className="h-3 w-3" /></button></Badge>
                ))}
                {selectedVibes.map(id => {
                  ...
                  return (
                    <span key={`wish-chip-vibe-${id}`} className={`inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-xs-token font-medium ${theme.chipBgLight} dark:${theme.chipBgDark} ${theme.border} ${theme.textLight} dark:${theme.textDark}`}>
            ...
            {filtered.length === 0 && !loading ? (
              ...
              <>
                ...
                {displayMode==='compact' && (
                  <div className="space-y-1" role="list" aria-label="Wishlist oils list (compact)">
                    {filtered.map(o => (
...
interface WishlistModalOilDetailProps {
  oilId: number;
  onClose: () => void;
  oils: Oil[];
  wishlistIds: number[];
...
const WishlistModalOilDetail: React.FC<WishlistModalOilDetailProps> = ({ oilId, onClose, oils, wishlistIds, ownedIds, setWishlistIds, setOwnedIds, noteMap, aromaMap, vibeMap, setOils }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-24" role="dialog" aria-modal="true" aria-labelledby="wishlist-oil-modal-title">
      ...
      <div className="relative w-full max-w-xl mx-auto animate-overlay-enter" ref={containerRef}>
        <div className="rounded-2xl shadow-2xl bg-white/95 dark:bg-slate-900/90 backdrop-blur-md border border-white/30 dark:border-slate-700/60">
          <div className="flex items-center justify-between px-5 pt-4 pb-3 border-b border-slate-200 dark:border-slate-700">
            <h2 id="wishlist-oil-modal-title" className="text-md-token font-semibold text-slate-800 dark:text-slate-100">{o.name}</h2>
            ...
            <EssentialOilCard
              ...
              onStatusChange={(u)=>{
                if(u.wishlist !== undefined) {
                  ...
                  if(!u.wishlist) { setOils(prev => prev.filter(x=>x.id!==o.id)); onClose(); }
                  if(u.wishlist) setOwnedIds(prev => prev.filter(id=>id!==o.id));
                }
                if(u.owned !== undefined) {
                  setOwnedIds(prev => u.owned ? [...new Set([...prev, o.id])] : prev.filter(id=>id!==o.id));
                  if(u.owned) { setWishlistIds(prev => prev.filter(id=>id!==o.id)); setOils(prev => prev.filter(x=>x.id!==o.id)); onClose(); }
                }
              }}
              hideTitle={true}
            />
