...
import { Droplets, Search, Filter, X, Trash2, Loader2 } from 'lucide-react';
...
const EditBlend: React.FC = () => {
  const navigate = useNavigate();
  const { id } = useParams<{ id: string }>();
  const { success, error: toastError } = useToast();
  
  // Loading state for existing blend
  const [loadingBlend, setLoadingBlend] = useState(true);
  const [blendNotFound, setBlendNotFound] = useState(false);
  ...
  useEffect(()=>{
    ...
    client.get('/essential-oils/')
      ...
      .catch(e=>{ if(mounted){ setError('Failed loading oils'); } })
  ...
  // Load existing blend data
  useEffect(()=>{
    if (!id) return;
    setLoadingBlend(true);
    client.get(`/blends/${id}/`)
      .then(r => {
        const blend = r.data;
        setName(blend.name || '');
        setDescription(blend.description || '');
        setIsPublic(blend.is_public ?? true);
        // Convert ingredients_detail to IngredientDraft format
        // ingredients_detail has: { oil: { id, name, notes, aromas, vibes }, drops, note: { id, name, label } }
        const ingredientsList: IngredientDraft[] = (blend.ingredients_detail || []).map((ing: any) => ({
          oil: {
            id: ing.oil?.id,
            name: ing.oil?.name,
            notes: ing.oil?.notes || [],
            aromas: ing.oil?.aromas || [],
            vibes: ing.oil?.vibes || [],
          },
          drops: ing.drops,
          noteId: ing.note?.id || null,
        }));
        setIngredients(ingredientsList);
      })
      .catch((e) => {
        console.error('Failed to load blend:', e);
        if (e.response?.status === 404 || e.response?.status === 403) {
          setBlendNotFound(true);
        } else {
          toastError('Failed to load blend');
        }
      })
      .finally(() => setLoadingBlend(false));
  }, [id]);
  ...
  const addIngredient = (oil:Oil) => {
    if(ingredients.some(i=>i.oil.id===oil.id)) return;
  ...
  const submitBlend = async () => {
    ...
    try {
      ...
      await client.put(`/blends/${id}/`, payload);
      success(`"${name.trim()}" updated successfully!`);
      navigate('/blends/my-blends');
    } catch(e:any){
      console.error(e); 
      const errMsg = e?.response?.data ? JSON.stringify(e.response.data) : 'Failed updating blend';
      setSubmitError(errMsg);
      toastError('Failed to update blend');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  if (loadingBlend) {
    return (
      <div className="py-4">
        <PageHeader title="Edit Blend" icon={<Droplets className="h-6 w-6" />} subtitle="Loading blend..." />
        <Surface elevation={1} className="p-8 flex items-center justify-center gap-3">
          <Loader2 className="h-5 w-5 animate-spin text-teal-600" />
          <span className="text-sm text-slate-600 dark:text-slate-300">Loading blend data...</span>
        </Surface>
      </div>
    );
  }
  ...
  if (blendNotFound) {
    return (
      <div className="py-4">
        <PageHeader title="Edit Blend" icon={<Droplets className="h-6 w-6" />} subtitle="Blend not found" />
        <Surface elevation={1} className="p-8 text-center">
          <p className="text-slate-600 dark:text-slate-300 mb-4">This blend doesn't exist or you don't have permission to edit it.</p>
          <Button onClick={() => navigate('/blends/my-blends')}>Back to My Blends</Button>
        </Surface>
      </div>
    );
  }
          ...
          {(() => {
            ...
            return (
              <div className="p-5 space-y-4">
                <EssentialOilCard id={o.id} name={o.name} notes={o.notes} aromas={o.aromas} vibes={o.vibes} description={o.description} noteMap={noteMap} aromaMap={aromaMap} vibeMap={vibeMap} hideTitle={true} />
  ...
  return (
    <div>
      <PageHeader title="Edit Blend" icon={<Droplets className="h-6 w-6" />} subtitle="Modify your blend's oils, drops, and settings." />
        ...
        <div className="lg:col-span-1 space-y-6">
          <Surface elevation={1} className="p-4 space-y-4">
            ...
            <div className="flex gap-2">
              <Button variant="outline" onClick={() => navigate('/blends/my-blends')} className="flex-1">Cancel</Button>
              <Button disabled={!canSubmit || submitting} onClick={submitBlend} className="flex-1" variant="default">{submitting ? 'Saving...' : 'Save Changes'}</Button>
            </div>
            ...
            <div className="flex-1 grid lg:grid-cols-4 gap-0">
              <div className="lg:col-span-3 p-4">
                ...
                {!loadingOils && !error && filteredOils.length===0 && (
                  <div className="text-xs text-slate-500">No oils available (or no oils match current filters).</div>
                )}
